...
const (
	ProgressAnimColors  [7 - 0 + 1]byte   = [...]string{0x14, 0x1C, 0x15, 0x1D, 0x16, 0x1E, 0x17, 0x1F}
	ProgressAnimStrings [7 - 0 + 1]string = [...]string{"....|", "...*/", "..*.-", ".*..\\", "*...|", "..../", "....-", "....\\"}
	ColorNames          [7 - 1 + 1]string = [...]string{"Blue", "Green", "Cyan", "Red", "Purple", "Yellow", "White"}
	DiagonalDeltaX      [7 - 0 + 1]int16  = [...]string{-1, 0, 1, 1, 1, 0, -1, -1}
	DiagonalDeltaY      [7 - 0 + 1]int16  = [...]string{1, 1, 1, 0, -1, -1, -1, 0}
	NeighborDeltaX      [3 - 0 + 1]int16  = [...]string{0, 0, -1, 1}
	NeighborDeltaY      [3 - 0 + 1]int16  = [...]string{-1, 1, 0, 0}
...
func SidebarPromptCharacter(editable bool, x, y int16, prompt TString50, value *byte) {
	...
	for {
		for i := (value - 4); i <= (value + 4); i++ {
			VideoWriteText(((x+i)-value)+5, y+2, 0x1E, Chr((i+0x100)%0x100))
		}
		if editable {
			Delay(25)
			InputUpdate()
...
func SidebarPromptSlider(editable bool, x, y int16, prompt string, value *byte) {
	...
	for {
		if editable {
			...
			VideoWriteText(x+value+1, y+1, 0x9F, '\x1f')
			InputUpdate()
	...
	VideoWriteText(x+value+1, y+1, 0x1F, '\x1f')
}
...
func SidebarPromptChoice(editable bool, y int16, prompt, choiceStr string, result *byte) {
	...
	SidebarClearLine(y + 1)
	SidebarClearLine(y + 2)
	VideoWriteText(63, y, int16(editable)+0x1E, prompt)
...
func PromptString(x, y, arrowColor, color, width int16, mode byte, buffer *TString50) {
	...
	for {
		...
		VideoWriteText(x+Length(buffer), y-1, (arrowColor/0x10)*16+0x0F, '\x1f')
...
func GamePlayLoop(boardChanged bool) {
	...
	for {
		if GamePaused {
			...
			if pauseBlink {
				VideoWriteText(Board.Stats[0].X-1, Board.Stats[0].Y-1, ElementDefs[E_PLAYER].Color, ElementDefs[E_PLAYER].Character)
			} else {
